...
export default class TestScene extends BaseScene {
  ...
  constructor(engine: Engine, canvas: any) {
    ...
    console.log(
      `this.player.compoundMesh.position:${this.player.compoundMesh.position}`
    );

    // This creates a light, aiming 0,1,0 - to the sky (non-mesh)
    var light = new HemisphericLight("light", new Vector3(0, 1, 0), this);

    let camera = new UFICamera(
      this,
      this.canvas,
      this.player.compoundMesh.position
    );
    ...
    let ball2 = MeshBuilder.CreateSphere(
      "",
      { diameter: 1, segments: 4 },
      this
    );
    let ball3 = MeshBuilder.CreateSphere(
      "",
      { diameter: 1, segments: 4 },
      this
    );

    let ball4 = MeshBuilder.CreateSphere(
      "",
      { diameter: 1, segments: 4 },
      this
    );
    let ball5 = MeshBuilder.CreateSphere(
      "",
      { diameter: 1, segments: 4 },
      this
    );
    ...
    this.controller = new AnimatedController(
      ...
      this.canvas,
      [PLAYER_IDLE],
      [PLAYER_WALKING_F1, PLAYER_WALKING_F2],
      [PLAYER_WALKING_B1, PLAYER_WALKING_B2],
      [PLAYER_WALKING_L1, PLAYER_WALKING_L2],
      [PLAYER_WALKING_R1, PLAYER_WALKING_R2]
    );
    this.player.addAnimation((<AnimatedController>this.controller).idleAnim);
    this.player.addAnimation(
      (<AnimatedController>this.controller).walkingFAnim
    );
    this.player.addAnimation(
      (<AnimatedController>this.controller).walkingBAnim
    );
    this.player.addAnimation(
      (<AnimatedController>this.controller).walkingLAnim
    );
    this.player.addAnimation(
      (<AnimatedController>this.controller).walkingRAnim
    );
